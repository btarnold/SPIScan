...
static const scan_type st[] = {
	SCAN_TYPE(300,100),		// placement check
...
fflush(stdout);
				...
				if (strcmp(o->name, "depth") == 0) { // set the pixel component size to 8
					SANE_Word val_word = 8;
					sane_control_option(handle, i, SANE_ACTION_SET_VALUE, &val_word, 0);
				} else
...
printf("start\n");
		res = sane_start(handle);
		if (res != SANE_STATUS_GOOD) {
		        if(res == SANE_STATUS_NO_MEM) { printf("\nSANE_STATUS_NO_MEM returned from sane_start()\n"); }
		        if(res == SANE_STATUS_CANCELLED) { printf("\nSANE_STATUS_CANCELLED returned from sane_start()\n"); }
		        if(res == SANE_STATUS_DEVICE_BUSY) { printf("\nSANE_STATUS_DEVICE_BUSY returned from sane_start()\n"); }
		        if(res == SANE_STATUS_JAMMED) { printf("\nSANE_STATUS_JAMMED returned from sane_start()\n"); }
		        if(res == SANE_STATUS_NO_DOCS) { printf("\nSANE_STATUS_NO_DOCS returned from sane_start()\n"); }
		        if(res == SANE_STATUS_IO_ERROR) { printf("\nSANE_STATUS_IO_ERROR returned from sane_start()\n"); }
		        if(res == SANE_STATUS_COVER_OPEN) { printf("\nSANE_STATUS_COVER_OPEN returned from sane_start()\n"); }
		        if(res == SANE_STATUS_INVAL) { printf("\nSANE_STATUS_INVAL returned from sane_start()\n"); }
			printf("retrying");
			goto retry;
		}
printf("start done\n");
...
// purpose
void
scan_start(int type, char *filename)
{
...
printf("jpeg create compress\n");
			...
			outfile = fopen(file_name, "w"); // opens file for jpg version of last scan
			if (!outfile) {
printf("open failed\n");
				jpeg_active = 0;
				jpeg_destroy_compress(&cinfo);
